...
//
// Postfix_Flyweight_Performance_Evaluator_Strategy
// Default Constructor.
//
Postfix_Flyweight_Performance_Evaluator_Strategy::Postfix_Flyweight_Performance_Evaluator_Strategy()
{
...
//
// ~Postfix_Flyweight_Performance_Evaluator_Strategy
// Destructor which is called when Postfix_Flyweight_Performance_Evaluator_Strategy objects are destroyed (deallocated).
//
Postfix_Flyweight_Performance_Evaluator_Strategy::~Postfix_Flyweight_Performance_Evaluator_Strategy()
{
...
//
bool Postfix_Flyweight_Performance_Evaluator_Strategy::parseExpr(const std::string & infix, int &result)
{
	...
	{	
		...
		for (size_t i = 0; i < postfix.size(); i++)
		{
			...
			if(postfix[i]->getTokenValue() == "number")
			{
				delete postfix[i];
			}
			arraySize --;
	...
	catch (std::exception & e)
	{
		...
		if (arraySize != 0)
		{
			if(postfix[arrayStartIndex]->getTokenValue()== "number")
			{
...
//
bool Postfix_Flyweight_Performance_Evaluator_Strategy :: convertInfixToPostfix (const std::string & infix, Expr_Command_Factory & factory, Array <Expr_Command *> & postfix)
{
...
//
int Postfix_Flyweight_Performance_Evaluator_Strategy :: getOperatorPriority(std::string objOperator)
{
	...
	int priority = 0; 
	if((objOperator == "+" )|| (objOperator == "-" ))
	{
		priority = 1;
	}
	else if((objOperator == "*" ) || (objOperator == "/" ) || (objOperator == "%" ))
	{
		priority = 2;
	}
